...
from modifyinput import invert, get_language_specific_positions, shift_special_tokens, replace_with_nn, replace_with_dict
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args, model: PreTrainedModel) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if args.replace_with_nn > 0:
        ...
        replace_with_nn(inputs, model, indices_random, args.replace_with_nn, vecmap_space=args.vecmap, tok=tokenizer)
    if args.use_anchor_points:
        indices_random = torch.bernoulli(torch.full(labels.shape, args.replacement_probs[2])).bool() & masked_indices & ~indices_replaced
        replace_with_dict(inputs, model, indices_random)

    if args.crosslingual_context > 0:
        probability_matrix_context = probability_matrix.clone()
        probability_matrix_context.masked_fill_(masked_indices, value=0.0)
        probability_matrix_context.masked_fill_(probability_matrix_context > 0.0, value=args.crosslingual_context)
        context_word_indices = torch.bernoulli(probability_matrix_context).bool()
        replace_with_nn(inputs, model, context_word_indices, 1)
...
def main():
    ...
    if args.add_fake_english and args.do_train:
    ...
    if args.delete_position_segment_embeddings and args.do_train:
